...
#define STR_COPYRIGHT	"mconv.exe (c) 2010-2012 Ver 0.30 By 59414d41\n\n"
...
void fftFilter(int type,DWORD inSample1,DWORD inSample2,FIO *fio1,FIO *fio2,FIO *fio3,PARAM_INFO *param);
void fftFilterSub(int type,SSIZE *pIn,SSIZE *pOut,fftw_complex *fftw_in,fftw_complex *fftw_out,fftw_plan fftw_p,fftw_plan fftw_ip,PARAM_INFO *param,int id);
 ...
 */
{
	...
	do {
		...
		_makepath(outFile,drive,dir,fname,"r1");
		...
		// Front Right
		_splitpath(paramFile,drive,dir,fname,ext);
		_makepath(outFile,drive,dir,fname,"r2");
		fio_open(&fp_r,outFile,FIO_MODE_R);
		if (fp_r.error) {
			param->err = STATUS_FILE_READ_ERR;param->error_line = __LINE__;
			break;
		}
		fio_set_memory_limit(&fp_r,10);
		inSample2 = 0;
		// ファイルサイズ取得
		...
		if (param->chC == 1) {
			...
			_splitpath(paramFile,drive,dir,fname,ext);
			_makepath(outFile,drive,dir,fname,"r3");
		...
		if (param->chS == 1) {
			...
			_splitpath(paramFile,drive,dir,fname,ext);
			_makepath(outFile,drive,dir,fname,"r4");
			fio_open(&fp_w,outFile,FIO_MODE_W);
			if (fp_w.error) {
				param->err = STATUS_FILE_WRITE_ERR;param->error_line = __LINE__;
				break;
			...
			_splitpath(paramFile,drive,dir,fname,ext);
			_makepath(outFile,drive,dir,fname,"r5");
			fio_open(&fp_w,outFile,FIO_MODE_W);
			if (fp_w.error) {
				param->err = STATUS_FILE_WRITE_ERR;param->error_line = __LINE__;
				break;
		...
		if (param->chLFE == 1) {
			...
			_splitpath(paramFile,drive,dir,fname,ext);
			_makepath(outFile,drive,dir,fname,"r6");
			fio_open(&fp_w,outFile,FIO_MODE_W);
			if (fp_w.error) {
				param->err = STATUS_FILE_WRITE_ERR;param->error_line = __LINE__;
				break;
		...
		if (param->downmix) {
			if (param->chC == 1) {
				...
				_splitpath(paramFile,drive,dir,fname,ext);
				_makepath(outFile,drive,dir,fname,"r1.tmp2");
				fio_open(&fp_w,outFile,FIO_MODE_W);
				if (fp_w.error) {
					param->err = STATUS_FILE_READ_ERR;param->error_line = __LINE__;
					break;
				...
				_splitpath(paramFile,drive,dir,fname,ext);
				_makepath(outFile,drive,dir,fname,"r2.tmp2");
				fio_open(&fp_w,outFile,FIO_MODE_W);
				if (fp_w.error) {
					param->err = STATUS_FILE_READ_ERR;param->error_line = __LINE__;
					break;
		...
		memset(&NormInfo,0,sizeof (NORM_INFO));
		_splitpath(paramFile,drive,dir,fname,ext);
		_makepath(outFile,drive,dir,fname,"r1.tmp");
		fio_open(&fp_w,outFile,FIO_MODE_W);
		if (fp_w.error) {
			param->err = STATUS_FILE_WRITE_ERR;param->error_line = __LINE__;
			break;
		...
		avg = NormInfo.avg << 40;
		_splitpath(paramFile,drive,dir,fname,ext);
		_makepath(outFile,drive,dir,fname,"r1.param");
		fp = fopen(outFile,"wb");
		if (fp == NULL) {
			param->err = STATUS_FILE_WRITE_ERR;param->error_line = __LINE__;
			break;
		...
		memset(&NormInfo,0,sizeof (NORM_INFO));
		_splitpath(paramFile,drive,dir,fname,ext);
		_makepath(outFile,drive,dir,fname,"r2.tmp");
		fio_open(&fp_w,outFile,FIO_MODE_W);
		if (fp_w.error) {
			param->err = STATUS_FILE_WRITE_ERR;param->error_line = __LINE__;
			break;
		...
		avg = NormInfo.avg << 40;
		_splitpath(paramFile,drive,dir,fname,ext);
		_makepath(outFile,drive,dir,fname,"r2.param");
		fp = fopen(outFile,"wb");
		if (fp == NULL) {
			param->err = STATUS_FILE_WRITE_ERR;param->error_line = __LINE__;
			break;
		...
		if (param->chC == 1) {
			...
			_splitpath(paramFile,drive,dir,fname,ext);
			_makepath(outFile,drive,dir,fname,"r3.tmp");
			fio_open(&fp_w,outFile,FIO_MODE_W);
			if (fp_w.error) {
				param->err = STATUS_FILE_WRITE_ERR;param->error_line = __LINE__;
				break;
			...
			avg = NormInfo.avg << 40;
			_splitpath(paramFile,drive,dir,fname,ext);
			_makepath(outFile,drive,dir,fname,"r3.param");
			fp = fopen(outFile,"wb");
			if (fp == NULL) {
				param->err = STATUS_FILE_WRITE_ERR;param->error_line = __LINE__;
				break;
		...
		if (param->chS == 1) {
			...
			_splitpath(paramFile,drive,dir,fname,ext);
			_makepath(outFile,drive,dir,fname,"r4.tmp");
			fio_open(&fp_w,outFile,FIO_MODE_W);
			if (fp_w.error) {
				param->err = STATUS_FILE_WRITE_ERR;param->error_line = __LINE__;
				break;
			...
			avg = NormInfo.avg << 40;
			_splitpath(paramFile,drive,dir,fname,ext);
			_makepath(outFile,drive,dir,fname,"r4.param");
			fp = fopen(outFile,"wb");
			if (fp == NULL) {
				param->err = STATUS_FILE_WRITE_ERR;param->error_line = __LINE__;
				break;
			...
			_splitpath(paramFile,drive,dir,fname,ext);
			_makepath(outFile,drive,dir,fname,"r5.tmp");
			fio_open(&fp_w,outFile,FIO_MODE_W);
			if (fp_w.error) {
				param->err = STATUS_FILE_WRITE_ERR;param->error_line = __LINE__;
				break;
			...
			avg = NormInfo.avg << 40;
			_splitpath(paramFile,drive,dir,fname,ext);
			_makepath(outFile,drive,dir,fname,"r5.param");
			fp = fopen(outFile,"wb");
			if (fp == NULL) {
				param->err = STATUS_FILE_WRITE_ERR;param->error_line = __LINE__;
				break;
		...
		if (param->chLFE == 1) {
			...
			_splitpath(paramFile,drive,dir,fname,ext);
			_makepath(outFile,drive,dir,fname,"r6.tmp");
			fio_open(&fp_w,outFile,FIO_MODE_W);
			if (fp_w.error) {
				param->err = STATUS_FILE_WRITE_ERR;param->error_line = __LINE__;
				break;
			...
			avg = NormInfo.avg << 40;
			_splitpath(paramFile,drive,dir,fname,ext);
			_makepath(outFile,drive,dir,fname,"r6.param");
			fp = fopen(outFile,"wb");
			if (fp == NULL) {
				param->err = STATUS_FILE_WRITE_ERR;param->error_line = __LINE__;
				break;
