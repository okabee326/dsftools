...
void fio_seek(FIO *fio,fio_size offset,int orign)
{
	...
	if (error && fio->ignore_rd_error == 0) {
		fio->error = FIO_ERR_OTHER;
		PRINT_LOG("ERROR");
	}
}
//---------------------------------------------------------------------------
// Function   : fio_set_ignore_rd_error
// Description: ファイル読み込み時のエラーを無視するフラグ設定
// ---
//	fio			: fileio構造体
//	flag		: エラー無視(1)、クリア(0)
//
void fio_set_ignore_rd_error(FIO *fio,int flag)
{
	if (fio == NULL) {
		PRINT_LOG("ERROR");
		return;
	...
	if (fio->magic != 5915 || flag < 0 || flag > 1) {
		fio->error = FIO_ERR_OTHER;
		PRINT_LOG("ERROR");
		return;
	}
	fio->ignore_rd_error = flag;
...
//PRINT_LOG("find block:none");
		...
		if (p_data->block_no != block) {
			...
			if (error) {
				sprintf(s,"data_size:%lld\n",fio->data_size);
				PRINT_LOG(s);
				sprintf(s,"data_maxsize:%lld\n",fio->data_maxsize);
				PRINT_LOG(s);
				sprintf(s,"tmp_offset:%lld\n",tmp_offset);
				PRINT_LOG(s);
				sprintf(s,"tmp_size:%lld\n",tmp_size);
				PRINT_LOG(s);
				sprintf(s,"rd:%lld\n",rd);
				PRINT_LOG(s);
				PRINT_LOG("read zero");
...
void fio_fill_buffer(FIO *fio)
{
	...
	if (fio->data_size < offset || (fio->data_size - offset) < 0) {
		return;
	}
	...
	while (p_data != NULL) {
		...
		} else if (fio->ignore_rd_error == 1) {
			return;
		}
		if (error) {
			char s[100];
			sprintf(s,"offset:%lld,size:%lld\n",fio->data_offset,fio->data_size);
			PRINT_LOG(s);
			PRINT_LOG(fio->fname);
			PRINT_LOG("ERROR");
			fio->error = FIO_ERR_READ;
